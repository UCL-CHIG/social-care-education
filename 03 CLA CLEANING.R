...
# length(unique(cla_children$cla_child_id_anon)) 
# length(unique(cla_children$cla_child_la_code_anon)) 
# the latter is used for different children in different LAs so we should probably use the first

ids <- unique(cla_children[, c("cla_child_id_anon", "cla_cla_la")])
any(duplicated(ids$cla_child_id_anon)) # no
rm(ids)
...
# THE FOLLOWING IS CODE FROM A PREVIOUS EXTRACT AND IS NO LONGER RELEVANT.

# # first need to ensure that PMRs not shared across different child IDs
# cla_children$child_id_int <- as.integer(as.factor(cla_children$cla_child_id_anon))
# 
# cla_children[!is.na(pmr2), pmr.min := min(as.integer(child_id_int)), by = "pmr2"]
# cla_children[!is.na(pmr2), pmr.max := max(as.integer(child_id_int)), by = "pmr2"]
# cla_children[!is.na(pmr2), pmr.check := pmr.min != pmr.max]
# table(cla_children$pmr.check) # 
# 
# length(unique(cla_children[pmr.check == TRUE]$child_id_int)) # 
# length(unique(cla_children[pmr.check == TRUE]$pmr2)) # 
# 
# cla_children[!is.na(pmr2) & pmr.check == TRUE, dob.min := min(cla_dob), by = "pmr2"]
# cla_children[!is.na(pmr2) & pmr.check == TRUE, dob.max := max(cla_dob), by = "pmr2"]
# cla_children[!is.na(pmr2) & pmr.check == TRUE, dob.check := dob.min != dob.max]
# table(cla_children$dob.check) # 
# 
# cla_children[!is.na(pmr2) & pmr.check == TRUE, fem.min := min(cla_sex), by = "pmr2"]
# cla_children[!is.na(pmr2) & pmr.check == TRUE, fem.max := max(cla_sex), by = "pmr2"]
# cla_children[!is.na(pmr2) & pmr.check == TRUE, fem.check := fem.min != fem.max]
# table(cla_children$fem.check) # 
# 
# cla_children[!is.na(pmr2) & pmr.check == TRUE, eth.min := min(as.integer(as.factor(cla_ethnic))), by = "pmr2"]
# cla_children[!is.na(pmr2) & pmr.check == TRUE, eth.max := max(as.integer(as.factor(cla_ethnic))), by = "pmr2"]
# cla_children[!is.na(pmr2) & pmr.check == TRUE, eth.check := eth.min != eth.max]
# table(cla_children$eth.check) # 
# 
# cla_children$child_id_int_2 <- cla_children$child_id_int
# 
# cla_children <- cla_children[order(pmr2, cla_child_id_anon)]
# 
# # rule 1: if DOB, gender and ethnicity all match, assume same child and copy earliest child ID down
# nrow(cla_children[pmr.check == TRUE & dob.check == FALSE & fem.check == FALSE & eth.check == FALSE])
# # 
# 
# cla_children[pmr.check == TRUE & dob.check == FALSE & fem.check == FALSE & eth.check == FALSE,
#              child_id_int_2 := min(child_id_int), by = "pmr2"]
# 
# cla_children[!is.na(pmr2), pmr.min := min(child_id_int_2), by = "pmr2"]
# cla_children[!is.na(pmr2), pmr.max := max(child_id_int_2), by = "pmr2"]
# cla_children[!is.na(pmr2), pmr.check := pmr.min != pmr.max]
# table(cla_children$pmr.check) # 
# 
# # rule 2: if DOB differs
# nrow(cla_children[pmr.check == TRUE & dob.check == TRUE & fem.check == FALSE & eth.check == FALSE])
# 
# cla_children[is.na(pmr2)]$pmr2 <- "NA"
# 
# pmr.check <- cla_children$pmr.check
# dob.check <- cla_children$dob.check
# fem.check <- cla_children$fem.check
# eth.check <- cla_children$eth.check
# child_id_int_2 <- cla_children$child_id_int_2
# cla_child_id_anon <- cla_children$cla_child_id_anon
# pmr2 <- cla_children$pmr2
# dob <- cla_children$cla_dob
# pb <- txtProgressBar(min = 0, max = length(child_id_int_2), style = 3)
# set.seed(1066)
# 
# for (i in 1:length(child_id_int_2)) {
#   if (!is.na(pmr.check[i]) & !is.na(dob.check[i]) & pmr.check[i] & dob.check[i] &
#       !fem.check[i] & !eth.check[i]) {
#     dob.vec <- dob[pmr2 == pmr2[i]]
#     ux <- unique(dob.vec)
#     tab <- tabulate(match(dob.vec, ux))
#     md <- ux[tab == max(tab)]
#     if (length(md) == 1) {
#       dob[pmr2 == pmr2[i]] <- md
#       child_id_int_2[pmr2 == pmr2[i]] <- min(child_id_int_2[pmr2 == pmr2[i]])
#       cla_child_id_anon[pmr2 == pmr2[i]] <- min(cla_child_id_anon[pmr2 == pmr2[i]])
#       dob.check[pmr2 == pmr2[i]] <- FALSE
#     } else {
#       dob[pmr2 == pmr2[i]] <- md[which(rmultinom(1, 1, rep(1/length(md), length(md))) == 1)]
#       child_id_int_2[pmr2 == pmr2[i]] <- min(child_id_int_2[pmr2 == pmr2[i]])
#       cla_child_id_anon[pmr2 == pmr2[i]] <- min(cla_child_id_anon[pmr2 == pmr2[i]])
#       dob.check[pmr2 == pmr2[i]] <- FALSE
#     }
...
# }
# 
# cla_children$cla_child_id_anon <- cla_child_id_anon
# cla_children$child_id_int_2 <- child_id_int_2
# cla_children$cla_dob <- dob
# 
# rm(pb, i, child_id_int_2, dob, pmr.check, dob.check, pmr2,
#    dob.vec, md, tab, ux, fem.check, eth.check, cla_child_id_anon)
# 
# cla_children[pmr2 == "NA"]$pmr2 <- NA
# 
# cla_children[!is.na(pmr2), pmr.min := min(child_id_int_2), by = "pmr2"]
# cla_children[!is.na(pmr2), pmr.max := max(child_id_int_2), by = "pmr2"]
# cla_children[!is.na(pmr2), pmr.check := pmr.min != pmr.max]
# table(cla_children$pmr.check) # 
# 
# # rule 3: gender differs but other two same
# nrow(cla_children[pmr.check == TRUE & dob.check == FALSE & fem.check == TRUE & eth.check == FALSE]) # 
# 
# cla_children[is.na(pmr2)]$pmr2 <- "NA"
# 
# pmr.check <- cla_children$pmr.check
...
# cla_child_id_anon <- cla_children$cla_child_id_anon
# pmr2 <- cla_children$pmr2
# female <- cla_children$female
# pb <- txtProgressBar(min = 0, max = length(child_id_int_2), style = 3)
# set.seed(1215)
# 
# for (i in 1:length(child_id_int_2)) {
#   if (!is.na(pmr.check[i]) & !is.na(dob.check[i]) & pmr.check[i] &
#       !dob.check[i] & fem.check[i] & !eth.check[i]) {
#     fem.vec <- female[pmr2 == pmr2[i]]
#     ux <- unique(fem.vec)
#     tab <- tabulate(match(fem.vec, ux))
#     md <- ux[tab == max(tab)]
#     if (length(md) == 1) {
#       female[pmr2 == pmr2[i]] <- md
#       child_id_int_2[pmr2 == pmr2[i]] <- min(child_id_int_2[pmr2 == pmr2[i]])
#       cla_child_id_anon[pmr2 == pmr2[i]] <- min(cla_child_id_anon[pmr2 == pmr2[i]])
#       dob.check[pmr2 == pmr2[i]] <- FALSE
#     } else {
#       female[pmr2 == pmr2[i]] <- md[which(rmultinom(1, 1, rep(1/length(md), length(md))) == 1)]
#       child_id_int_2[pmr2 == pmr2[i]] <- min(child_id_int_2[pmr2 == pmr2[i]])
#       cla_child_id_anon[pmr2 == pmr2[i]] <- min(cla_child_id_anon[pmr2 == pmr2[i]])
#       dob.check[pmr2 == pmr2[i]] <- FALSE
#     }
...
# cla_children$cla_child_id_anon <- cla_child_id_anon
# cla_children$child_id_int_2 <- child_id_int_2
# cla_children$female <- female
# 
# rm(pb, i, child_id_int_2, female, pmr.check, dob.check, pmr2,
#    fem.vec, md, tab, ux, fem.check, eth.check, cla_child_id_anon)
# 
# cla_children[pmr2 == "NA"]$pmr2 <- NA
# 
# cla_children[!is.na(pmr2), pmr.min := min(child_id_int_2), by = "pmr2"]
# cla_children[!is.na(pmr2), pmr.max := max(child_id_int_2), by = "pmr2"]
# cla_children[!is.na(pmr2), pmr.check := pmr.min != pmr.max]
# table(cla_children$pmr.check) # 4492 remaining
# 
# # rule 4: where ethnicity differs
# nrow(cla_children[pmr.check == TRUE & dob.check == FALSE & fem.check == FALSE & eth.check == TRUE]) #
# 
# cla_children[is.na(pmr2)]$pmr2 <- "NA"
# 
# pmr.check <- cla_children$pmr.check
...
#   if (!is.na(pmr.check[i]) & !is.na(dob.check[i]) & pmr.check[i] &
#       !dob.check[i] & !fem.check[i] & eth.check[i]) {
#     eth.vec <- eth_grp[pmr2 == pmr2[i]]
#     ux <- unique(eth.vec)
#     tab <- tabulate(match(eth.vec, ux))
#     md <- ux[tab == max(tab)]
#     if (length(md) == 1) {
#       eth_grp[pmr2 == pmr2[i]] <- md
#       child_id_int_2[pmr2 == pmr2[i]] <- min(child_id_int_2[pmr2 == pmr2[i]])
#       cla_child_id_anon[pmr2 == pmr2[i]] <- min(cla_child_id_anon[pmr2 == pmr2[i]])
#       dob.check[pmr2 == pmr2[i]] <- FALSE
#     } else {
#       eth_grp[pmr2 == pmr2[i]] <- md[which(rmultinom(1, 1, rep(1/length(md), length(md))) == 1)]
#       child_id_int_2[pmr2 == pmr2[i]] <- min(child_id_int_2[pmr2 == pmr2[i]])
#       cla_child_id_anon[pmr2 == pmr2[i]] <- min(cla_child_id_anon[pmr2 == pmr2[i]])
#       dob.check[pmr2 == pmr2[i]] <- FALSE
#     }
...
# cla_children$cla_child_id_anon <- cla_child_id_anon
# cla_children$child_id_int_2 <- child_id_int_2
# cla_children$eth_grp <- eth_grp
# 
# rm(pb, i, child_id_int_2, eth_grp, pmr.check, dob.check, pmr2,
#    fem.check, md, tab, ux, eth.vec, eth.check, cla_child_id_anon)
# 
# cla_children[pmr2 == "NA"]$pmr2 <- NA
# 
# 
...
# table(cla_children$pmr.check) 
# 
# length(unique(cla_children[pmr.check == TRUE]$child_id_int_2)) # 
# length(unique(cla_children[pmr.check == TRUE]$pmr2)) # 
# 
# cla_children[pmr.check == TRUE]$pmr2 <- NA
# 
# # final re-check
# cla_children[, pmr.min := NULL]
# cla_children[, pmr.max := NULL]
# cla_children[, pmr.check := NULL]
# cla_children[!is.na(pmr2), pmr.min := min(child_id_int_2), by = "pmr2"]
# cla_children[!is.na(pmr2), pmr.max := max(child_id_int_2), by = "pmr2"]
# cla_children[!is.na(pmr2), pmr.check := pmr.min != pmr.max]
# table(cla_children$pmr.check) # 0
# 
# # re-move check vars
# cla_children[, pmr.min := NULL]
# cla_children[, pmr.max := NULL]
# cla_children[, pmr.check := NULL]
# 
# cla_children[, dob.min := NULL]
# cla_children[, dob.max := NULL]
# cla_children[, dob.check := NULL]
# 
# cla_children[, fem.min := NULL]
# cla_children[, fem.max := NULL]
# cla_children[, fem.check := NULL]
# 
# cla_children[, eth.min := NULL]
# cla_children[, eth.max := NULL]
# cla_children[, eth.check := NULL]
...
cla_episodes <- cla_episodes[pmr2 %in% cohort1$pupilmatchingrefanonymous |
                               pmr2 %in% cohort2$pupilmatchingrefanonymous]

cla_episodes$cohort <- ifelse(cla_episodes$pmr2 %in% cohort1$pupilmatchingrefanonymous, 1, 2)
...
save(stb, file = "PROCESSED DATA/stb.rda")
...
# most are last year and, it turns out, the rest are last episodes in POC so no new action needed
# THE FOLLOWING IS CODE FROM A PREVIOUS EXTRACT AND IS NO LONGER RELEVANT.

# epi.end.missing.old <- epi.end.missing <- cla_episodes[pmr2 %in% cla_episodes[is.na(dateepisodeceased1)]$pmr2, 
#                                                        c("pmr2", "poc_start", "dateepisodestarted1", "dateepisodeceased1",
#                                                          "reasonepisodeceased1", "legal_status",
#                                                          "placement", "reasonforplacementchange1", "row_num")]
# 
# # extract vectors
# pmr2 <- epi.end.missing$pmr2
# poc_start <- epi.end.missing$poc_start
# dateepisodestarted1 <- epi.end.missing$dateepisodestarted1
# dateepisodeceased1 <- epi.end.missing$dateepisodeceased1
# reasonepisodeceased1 <- epi.end.missing$reasonepisodeceased1
# legal_status <- as.character(epi.end.missing$legal_status)
# placement <- as.character(epi.end.missing$placement)
# y2016 <- dateepisodestarted1 >= as.Date("2016-04-01")
# mark_delete <- rep(0, nrow(epi.end.missing))
# rule <- rep(0, nrow(epi.end.missing))
# 
# # rule 1 - entered care in 2016/17 - still in care at EOY
# 
# # rule 2 - subsequent start date is different to present start date; placement or legal status differ
# # join episodes together into one spell
# 
# # rule 3 - subsequent start date is different to present start date; but placement and legal 
# # status are the same
# # assume actual placement change and join together into one spell
# 
# # rule 4 - subsequent start date is the same to present start date
# # assume episode of 1 day's duration and fix date accordingly
# 
# # rule 5 - there is no subsequent episode
# # assume still in care
# 
# # loop through
# pb <- txtProgressBar(min = 1, max = nrow(epi.end.missing), style = 3)
# 
# for (i in 1:(nrow(epi.end.missing) - 1)) {
#   if (is.na(dateepisodeceased1[i])) {
#     if (!y2016[i]) { # rule 1
#       if (pmr2[i] == pmr2[i + 1]) {
#         if (poc_start[i] == poc_start[i + 1]) {
#           if (dateepisodestarted1[i] != dateepisodestarted1[i + 1]) {
#             if (legal_status[i] != legal_status[i + 1] |
#                 placement[i] != placement[i + 1]) {
#               dateepisodeceased1[i] <- dateepisodestarted1[i + 1] # rule 2
#               reasonepisodeceased1[i] <- "X1"
#               rule[i] <- 2
#             } else {
#               dateepisodeceased1[i] <- dateepisodestarted1[i + 1] # rule 3
#               reasonepisodeceased1[i] <- "X1"
#               rule[i] <- 3
#             }
#           } else {
#             dateepisodeceased1[i] <- dateepisodestarted1[i] + 1
#             dateepisodestarted1[i + 1] <- dateepisodestarted1[i + 1] + 1 # rule 4
#             rule[i] <- 4
#           }
#         } else {
#           rule[i] <- 5
...
# }
# 
# epi.end.missing$dateepisodestarted1 <- dateepisodestarted1
# epi.end.missing$dateepisodeceased1 <- dateepisodeceased1
# epi.end.missing$reasonepisodeceased1 <- reasonepisodeceased1
# epi.end.missing$mark_delete <- mark_delete
# epi.end.missing$rule <- rule
# 
# rm(i, pb, pmr2, poc_start, dateepisodestarted1, dateepisodeceased1, reasonepisodeceased1,
#    legal_status, placement, y2016, mark_delete, rule)
# 
# table(epi.end.missing$rule)
# 
# cla_episodes <- merge(cla_episodes, epi.end.missing[, c("row.num", "epi.start", "epi.end", "reason.end", "mark.delete")],
#                       by = "row.num",
#                       all.x = T)
# 
# cla_episodes[is.na(epi.start.y)]$epi.start.y <- cla_episodes[is.na(epi.start.y)]$epi.start.x
# cla_episodes[is.na(epi.end.y)]$epi.end.y <- cla_episodes[is.na(epi.end.y)]$epi.end.x
# 
# cla_episodes$reason.end.x <- as.factor(cla_episodes$reason.end.x)
# cla_episodes$reason.end.y <- as.factor(cla_episodes$reason.end.y)
# cla_episodes$reason.end.x <- factor(cla_episodes$reason.end.x,
#                                     levels = unique(c(levels(cla_episodes$reason.end.y),
#                                                       levels(cla_episodes$reason.end.x))))
# cla_episodes$reason.end.y <- factor(cla_episodes$reason.end.y,
#                                     levels = unique(c(levels(cla_episodes$reason.end.y),
#                                                       levels(cla_episodes$reason.end.x))))
# 
# cla_episodes[is.na(reason.end.y)]$reason.end.y <- cla_episodes[is.na(reason.end.y)]$reason.end.x
# cla_episodes <- cla_episodes[is.na(mark.delete) | mark.delete == 0]
# 
# cla_episodes[, epi.start.x := NULL]
# cla_episodes[, epi.end.x := NULL]
# cla_episodes[, reason.end.x := NULL]
# cla_episodes[, mark.delete := NULL]
# cla_episodes[, row.num := NULL]
# 
# setnames(cla_episodes,
#          c("epi.start.y", "epi.end.y", "reason.end.y"),
#          c("epi.start", "epi.end", "reason.end"))
# 
# rm(epi.end.missing, epi.end.missing.old)
...
# remove <- c("sex", "female", "eth_grp", "ethnic_or", "cla_child_id_anon", "max_epi_end", "reasonforplacementchange1")
# cla_episodes <- cla_episodes[, -remove, with = F]
# rm(remove)
...
save(cla_episodes, file = "PROCESSED DATA/cla_episodes.rda")
rm(list = ls()); gc()
